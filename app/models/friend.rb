class Friend < ApplicationRecord
  ...
  scope :friend_accept, lambda{|current_user_id|
    joins(:user).where("friends.status = ?", Settings.friend.request)
      .where("friends.friend_id = ?", current_user_id)
  }
  ...
  scope :check_friend, lambda{|status, user_id, current_user_id|
   where("friends.status = ?", status)
    .where("(friends.user_id = ? AND friends.friend_id = ?)OR (friends.friend_id = ? AND friends.user_id = ?)", user_id, current_user_id, user_id, current_user_id)
  }
  def accpet_request
